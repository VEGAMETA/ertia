...
static var by_flag : Dictionary = directions.reduce(func(accum:Dictionary, direction:GravityDirection): accum.merge({direction.flag:direction}); return accum, {})
static var by_vector: Dictionary =  directions.reduce(func(accum:Dictionary, direction:GravityDirection): accum.merge({direction.rotation_vector:direction}); return accum, {})
