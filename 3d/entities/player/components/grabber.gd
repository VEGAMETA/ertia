...
@export var grabbed_object : RigidBody3D
...
func grab() -> bool:
	...
	if grab_ray_collider == null: ungrab_object(false)
...
func throw_object() -> void:
	if grabbed_object != null and \
	Input.is_action_pressed("Alternative") and \
	Input.is_action_pressed("Attack"): ungrab_object(true)
...
func ungrab_object(throw: bool = false) -> void:
	if grabbed_object == null: return
	grabbed_object.linear_velocity /= 8.0 
	grabbed_object.angular_velocity /= 25.0
	if throw: grabbed_object.apply_central_impulse((THROW_FORCE + grabbed_object.mass * 2.0) * (grabbed_object.global_position - player.global_position))
	grabbed_object = null
